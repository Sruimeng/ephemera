...
export function StyleFilterProvider({
  ...
  const [config, setConfigState] = useState<PostProcessingConfig>(DEFAULT_POST_PROCESSING);
  ...
  const setFilter = useCallback((newFilter: StyleFilter) => {
  ...
  return <StyleFilterContext.Provider value={value}>{children}</StyleFilterContext.Provider>;
}
